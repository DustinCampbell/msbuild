...
internal struct PropertyFunctionParser
{
    ...
    /// </returns>
    public static ArgumentList ParseArguments(StringSegment argumentText, StringSegment expression, IElementLocation location)
    {
    ...
    private readonly int ScanForClosingDelimiter(
        ...
        PropertyFunctionErrorDetail mismatchedDelimiter)
    {
        int nestLevel = 1;
    ...
    private readonly int ScanForClosingQuote(StringSegment text, int index)
    {
        ...
        {
            _errors.InvalidPropertyFunction.Throw(_text, PropertyFunctionErrorDetail.MismatchedQuote);
        }
