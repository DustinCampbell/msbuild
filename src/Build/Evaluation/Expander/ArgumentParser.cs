...
internal static class ArgumentParser
{
        ...
        out StringComparison arg1)
    {
        ...
        if (args is [string value0, string value1] &&
            !TryParseInt(value1, out _))
        {
            ReadOnlySpan<char> span = value1.AsSpan();

            if (span.StartsWith("System."))
            {
                span = span["System.".Length..];
            }

            if (span.StartsWith("StringComparison."))
            {
                span = span["StringComparison.".Length..];
            }

#if NET
            if (Enum.TryParse(span, out arg1))
            {
                arg0 = value0;
                return true;
            }
#else
            if (Enum.TryParse(WeakIntern(span), out arg1))
            {
                arg0 = value0;
                return true;
            }
