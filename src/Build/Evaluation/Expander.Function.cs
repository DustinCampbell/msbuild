    ...
    private sealed class Function
    {
        ...
        internal object Execute(object objectInstance, ExpansionContext context)
        {
            ...
            try
            {
                ...
                {
                    WellKnownFunctionResult wellKnownFunctionResult = WellKnownFunctionResult.NotRecognized;
                    ...
                    if (wellKnownFunctionResult == WellKnownFunctionResult.InvalidArguments)
                    {
                        MissingMethodException exception = new(_receiverType.FullName, _methodName);
                    ...
                    if (wellKnownFunctionResult == WellKnownFunctionResult.NotRecognized)
                    {
                        if (args == null)
                        {
                            if (argumentMaterializer == null && arguments.Count > 0)
                        ...
                        functionResult = ReflectionInvoker.InvokeMember(_receiverType, _methodName, _bindingFlags, objectInstance, args);
                    }
                }
