...
namespace Microsoft.Build.Evaluation
{
    ...
    internal static class ExpressionShredder
    {
        ...
        public static int IndexOfPropertyMarker(string expression)
            => IndexOfMarker(expression, '$', 0, expression.Length);
        ...
        private static int IndexOfMarker(string expression, char marker, int startIndex, int count)
        {
            ...
            return expression[nextIndex] == '('
                ? markerIndex
                : IndexOfMarkerSlow(expression, marker, startIndex: nextIndex, count - (nextIndex - startIndex));
        }
        ...
        private static int IndexOfMarkerSlow(string expression, char marker, int startIndex)
        {
            ...
            while ((markerIndex = expression.IndexOf(marker, startIndex)) >= 0)
            {
        ...
        private static int IndexOfMarkerSlow(string expression, char marker, int startIndex, int count)
        {
            ...
            while ((markerIndex = expression.IndexOf(marker, startIndex, endIndex - startIndex)) >= 0)
            {
                startIndex = markerIndex + 1;

                if (startIndex < endIndex && expression[startIndex] == '(')
                {
                    return markerIndex;
                }
            }
