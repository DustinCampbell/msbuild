...
namespace Microsoft.Build.BackEnd
{
    ...
    internal class ItemGroupIntrinsicTask : IntrinsicTask
    {
        ...
        private void ExecuteAdd(ProjectItemGroupTaskItemInstance child, ItemBucket bucket, ISet<string> keepMetadata, ISet<string> removeMetadata, LoggingContext loggingContext = null)
        {
            ...
            NestedMetadataTable metadataTable = new(
                child.ItemType,
                bucket.Expander.CurrentMetadata,
                itemDefinition);

            using (bucket.Expander.EnterMetadataScope(metadataTable))
            {
                ...
                foreach (ProjectItemGroupTaskMetadataInstance metadataInstance in child.Metadata)
                {
                    ...
                    if (condition)
                    {
                        ...
                        string evaluatedValue = bucket.Expander.ExpandIntoStringLeaveEscaped(
                ...
                ProjectItemInstance.SetMetadata(metadataTable.AddedMetadata, itemsToAdd);
            }
