...
namespace Microsoft.Build.BackEnd
{
    ...
    internal abstract class IntrinsicTask
    {
        ...
        internal static IntrinsicTask InstantiateTask(ProjectTargetInstanceChild taskInstance, TargetLoggingContext loggingContext, ProjectInstance projectInstance, bool logTaskInputs)
            => taskInstance switch
            {
                ProjectPropertyGroupTaskInstance propertyGroup => new PropertyGroupIntrinsicTask(propertyGroup, loggingContext, projectInstance, logTaskInputs),
                ProjectItemGroupTaskInstance itemGroup => new ItemGroupIntrinsicTask(itemGroup, loggingContext, projectInstance, logTaskInputs),

                _ => Assumed.Unreachable<IntrinsicTask>($"Unhandled intrinsic task type {taskInstance.GetType().GetTypeInfo().BaseType}")
            };
