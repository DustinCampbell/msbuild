    ...
    internal class BuildRequestEntry
    {
        ...
        public void ReportResult(BuildResult result)
        {
            lock (GlobalLock)
            {
                ...
                {
                    Assumed.Unreachable($"Entry must be in the Waiting state to report results, or we must have flushed our requests due to an error. Config: {RequestConfiguration.ConfigurationId} State: {State} Requests: {_outstandingRequests != null}");
                }
