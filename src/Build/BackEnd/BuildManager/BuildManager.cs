                ...
                catch (Exception ex) when (IsInvalidProjectOrIORelatedException(ex))
                {
                    ...
                    lock (_syncLock)
                    {
                        ...
                        if (ex is not InvalidProjectFileException)
                        {
                            ...
                            ((IBuildComponentHost)this).LoggingService.LogFatalBuildError(buildEventContext, ex, BuildEventFileInfo.Create(submission.BuildRequestData.ProjectFullPath));
                        }
                    }
            ...
            if (shutdownPacket.Reason != NodeShutdownReason.Requested)
            {
                if (shutdownPacket.Reason == NodeShutdownReason.ConnectionFailed)
                {
                    ...
                    foreach (BuildSubmissionBase submission in _buildSubmissions.Values)
                    {
                        ...
                        loggingService?.LogError(buildEventContext, BuildEventFileInfo.Empty /* no project file */, "ChildExitedPrematurely", node, DebugUtils.DebugDumpPath, exception);
                    }
                }
                else if (shutdownPacket.Reason == NodeShutdownReason.Error && _buildSubmissions.Values.Count == 0)
                {
                    ...
                    {
                        ILoggingService loggingService = ((IBuildComponentHost)this).GetComponent<ILoggingService>(BuildComponentType.LoggingService);
                        loggingService?.LogError(BuildEventContext.Invalid, BuildEventFileInfo.Empty /* no project file */, "ChildExitedPrematurely", node, DebugUtils.DebugDumpPath, shutdownPacket.Exception.ToString());
                ...
                switch (response.Action)
                {
                    ...
                    case ScheduleActionType.CreateNode:
                        ...
                        if (newNodes?.Count != response.NumberOfNodesToCreate || newNodes.Any(n => n == null))
                        {
                            ...
                            ((IBuildComponentHost)this).LoggingService.LogError(buildEventContext, BuildEventFileInfo.Empty, "UnableToCreateNode", response.RequiredNodeType.ToString("G"));
