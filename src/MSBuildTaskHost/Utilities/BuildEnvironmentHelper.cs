...
internal sealed class BuildEnvironmentHelper
{
    ...
    private static BuildEnvironment Initialize()
    {
        ...
        if (CheckIfRunningTests())
        {
            msbuildExePath = typeof(BuildEnvironmentHelper).Assembly.Location;
        }
        else
        {
            msbuildExePath = GetProcessFromRunningProcess();
        }
    ...
    {
        string arch = NativeMethods.ProcessorArchitecture == ProcessorArchitecture.X64 ? "amd64" : string.Empty;
    ...
    private static readonly LockType s_runningTestsLock = new LockType();
...
internal sealed class BuildEnvironment
{
        ...
        string visualStudioPath)
    {
        ...
        if (mode == BuildEnvironmentMode.VisualStudio)
        {
            ...
            {
                ProcessorArchitecture.X86 => CurrentMSBuildToolsDirectory,
                ProcessorArchitecture.X64 => currentToolsDirectory.Parent?.FullName,

                var arch => throw new InternalErrorException($"Unexpected processor architecture {arch}"),
            };
        }
        else
        {
