...
using System.IO;
using Microsoft.Build.TaskHost.FileSystem;
...
internal static class ItemSpecModifiers
{
    ...
    private static readonly string[] s_definingProjectModifiers =
    [
    ...
    [
        FullPath,
            ...
            CreatedTime,
            AccessedTime,
            .. s_definingProjectModifiers
    ];

    private static readonly HashSet<string> s_tableOfItemSpecModifiers = new(All, StringComparer.OrdinalIgnoreCase);
    private static readonly HashSet<string> s_tableOfDefiningProjectModifiers = new(s_definingProjectModifiers, StringComparer.OrdinalIgnoreCase);
    ...
    internal static string GetItemSpecModifier(string currentDirectory, string itemSpec, string definingProjectEscaped, string modifier, ref string fullPath)
    {
        ...
        try
        {
            ...
            else if (string.Equals(modifier, ModifiedTime, StringComparison.OrdinalIgnoreCase))
            {
                ...
                modifiedItemSpec = FileSystems.Default.FileExists(unescapedItemSpec)
            ...
            else if (string.Equals(modifier, CreatedTime, StringComparison.OrdinalIgnoreCase))
            {
                ...
                modifiedItemSpec = FileSystems.Default.FileExists(unescapedItemSpec)
                    ? File.GetCreationTime(unescapedItemSpec).ToString(FileUtilities.FileTimeFormat)
                    : string.Empty; // File does not exist, or path is a directory
            }
            else if (string.Equals(modifier, AccessedTime, StringComparison.OrdinalIgnoreCase))
            {
                ...
                modifiedItemSpec = FileSystems.Default.FileExists(unescapedItemSpec)
                    ? File.GetLastAccessTime(unescapedItemSpec).ToString(FileUtilities.FileTimeFormat)
                    : string.Empty; // File does not exist, or path is a directory
            }
                ...
                else
                {
                    if (string.Equals(modifier, DefiningProjectDirectory, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        modifiedItemSpec = Path.Combine(
                            GetItemSpecModifier(currentDirectory, definingProjectEscaped, definingProjectEscaped: null, RootDir),
                            GetItemSpecModifier(currentDirectory, definingProjectEscaped, definingProjectEscaped: null, Directory));
                    }
                    else
                    {
