...
{
    private static Traits s_instance = new Traits();

    public static Traits Instance
        => BuildEnvironmentState.s_runningTests
            ? new Traits()
            : s_instance;
...
internal sealed class EscapeHatches
{
    ...
    {
        get => _logTaskInputs ??= Environment.GetEnvironmentVariable("MSBUILDLOGTASKINPUTS") == "1";
