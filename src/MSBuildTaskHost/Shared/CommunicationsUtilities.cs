        ...
        bool lowPriority = false)
    {
        ...
        if (taskHost)
        {
            ...
            if (taskHostParameters.IsEmpty)
            {
                ...
                architectureFlagToSet = MSBuildArchitecture.Current;
            }
            else // Figure out flags based on parameters given
            {
                ...
                if (!MSBuildRuntime.TryGetClrVersion(taskHostParameters.Runtime, out clrVersion))
                {
        ...
        if (string.Equals(architectureFlagToSet, MSBuildArchitecture.x64, StringComparison.OrdinalIgnoreCase))
        {
