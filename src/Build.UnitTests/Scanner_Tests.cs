...
namespace Microsoft.Build.UnitTests
{
    public class ScannerTest
    {
        ...
        {
            while (lexer.Advance() && !lexer.IsNext(Token.TokenKind.EndOfInput))
            {
                ;
            }
        }
        ...
        public void NumericSingleTokenTests()
        {
            ...
            Assert.True(lexer.IsNext(Token.TokenKind.Numeric));
        ...
        public void PropsStringsAndBooleanSingleTokenTests()
        {
            ...
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.Property));
            lexer = new Scanner("@(foo)", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.ItemList));
            lexer = new Scanner("abcde", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.String));
        ...
        public void SimpleSingleTokenTests()
        {
            Scanner lexer = new Scanner("(", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.LeftParenthesis));
            lexer = new Scanner(")", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.RightParenthesis));
            lexer = new Scanner(",", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.Comma));
            lexer = new Scanner("==", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.EqualTo));
            lexer = new Scanner("!=", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.NotEqualTo));
            lexer = new Scanner("<", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.LessThan));
            lexer = new Scanner(">", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.GreaterThan));
            lexer = new Scanner("<=", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.LessThanOrEqualTo));
            lexer = new Scanner(">=", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.GreaterThanOrEqualTo));
            lexer = new Scanner("!", ParserOptions.AllowAll);
            Assert.True(lexer.Advance());
            Assert.True(lexer.IsNext(Token.TokenKind.Not));
        }
        ...
        {
            Scanner lexer = new Scanner("@(Foo, ' ')", ParserOptions.AllowAll);
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.ItemList));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.EndOfInput));

            lexer = new Scanner("'@(Foo, ' ')'", ParserOptions.AllowAll);
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.String));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.EndOfInput));
            ...
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.String));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.EqualTo));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.String));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.EndOfInput));
        }
        ...
        {
            Scanner lexer = new Scanner("Foo()", ParserOptions.AllowAll);
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Function));
            Assert.Equal(0, String.Compare("Foo", lexer.IsNextString()));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.LeftParenthesis));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.RightParenthesis));

            lexer = new Scanner("Foo( 1 )", ParserOptions.AllowAll);
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Function));
            Assert.Equal(0, String.Compare("Foo", lexer.IsNextString()));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.LeftParenthesis));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Numeric));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.RightParenthesis));
            ...
            Assert.Equal(0, String.Compare("Foo", lexer.IsNextString()));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.LeftParenthesis));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Property));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.RightParenthesis));
            ...
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.LeftParenthesis));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.String));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Comma));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Numeric));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.RightParenthesis));
        ...
        public void ComplexTests2()
        {
            ...
            Assert.True(lexer.IsNext(Token.TokenKind.Property));
            lexer.Advance();
            Assert.True(lexer.IsNext(Token.TokenKind.EndOfInput));
            ...
            Assert.True(lexer.IsNext(Token.TokenKind.NotEqualTo));
            lexer.Advance();
            Assert.True(lexer.IsNext(Token.TokenKind.True));
            lexer.Advance();
            Assert.True(lexer.IsNext(Token.TokenKind.EndOfInput));
        ...
        public void WhitespaceTests()
        {
            ...
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Property));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.And));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Property));
            ...
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.ItemList));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.String));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.LessThan));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.GreaterThan));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.String));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.LessThanOrEqualTo));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.False));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.GreaterThanOrEqualTo));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.True));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.EqualTo));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.Numeric));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.NotEqualTo));
            Assert.True(lexer.Advance() && lexer.IsNext(Token.TokenKind.EndOfInput));
