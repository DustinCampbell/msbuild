...
public class PropertyFunctionParser_Tests
{
    ...
    public static TheoryData<string, int> ArgumentRequirementCases { get; } = new()
    {
        { "literal", (int)FunctionArgumentRequirements.None },
        { "null", (int)FunctionArgumentRequirements.None },
        { "''", (int)FunctionArgumentRequirements.None },
        { "@(Items)", (int)FunctionArgumentRequirements.None },
        { "%ZZ", (int)FunctionArgumentRequirements.None },
        { "$(Property)", (int)FunctionArgumentRequirements.ExpandProperties },
        { "%3B", (int)FunctionArgumentRequirements.Unescape },
        { "'$(Property)%3B'", (int)(FunctionArgumentRequirements.ExpandProperties | FunctionArgumentRequirements.Unescape) },
    };
    ...
    [Theory]
    [MemberData(nameof(ArgumentRequirementCases))]
    public void ClassifiesArgumentMaterializationRequirements(string argumentText, int expectedRequirements)
    {
        PropertyFunctionInvocation invocation = Parse($"[Type]::Method({argumentText})").Invocations[0];

        invocation.Arguments.Count.ShouldBe(1);
        invocation.Arguments.GetRequirements(0).ShouldBe((FunctionArgumentRequirements)expectedRequirements);
    }
