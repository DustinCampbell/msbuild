...
public class PropertyFunctionParser_Tests
{
    ...
    public static TheoryData<string, int> ArgumentFlagCases { get; } = new()
    {
        { "literal", (int)ArgumentFlags.None },
        { "null", (int)ArgumentFlags.None },
        { "''", (int)ArgumentFlags.None },
        { "@(Items)", (int)ArgumentFlags.None },
        { "%ZZ", (int)ArgumentFlags.None },
        { "$(Property)", (int)ArgumentFlags.ExpandProperties },
        { "prefix$(Property)", (int)ArgumentFlags.ExpandProperties },
        { "%3B", (int)ArgumentFlags.Unescape },
        { "%ZZ%3B", (int)ArgumentFlags.Unescape },
        {
            "$(Other.Method('%3B'))",
            (int)ArgumentFlags.All
        },
        { "'$(Property)%3B'", (int)ArgumentFlags.All },
    };
    ...
    public void ParsesStandaloneArguments(string argumentText, string?[] expectedArguments)
    {
        const string Prefix = "prefix";
        string buffer = $"{Prefix}{argumentText}suffix";
        StringSegment text = new(buffer, Prefix.Length, argumentText.Length);

        ArgumentList arguments = PropertyFunctionParser.ParseArguments(text, text, MockElementLocation.Instance);

        AssertArguments(arguments, expectedArguments);

        foreach (StringSegment argument in arguments)
        {
            if (argument.Length > 0)
            {
                argument.Buffer.ShouldBeSameAs(buffer);
            }
        }
    }

    [Theory]
    [MemberData(nameof(ArgumentFlagCases))]
    public void ClassifiesStandaloneArgumentFlags(string argumentText, int expectedFlags)
    {
        ArgumentList arguments = PropertyFunctionParser.ParseArguments(argumentText, argumentText, MockElementLocation.Instance);

        arguments.Count.ShouldBe(1);
        arguments.GetFlags(0).ShouldBe((ArgumentFlags)expectedFlags);
    }

    [Fact]
    public void PlainPropertyIsNotAPropertyFunction()
    {
        bool parsed = PropertyFunctionParser.TryParse("Configuration", MockElementLocation.Instance, out PropertyFunction expression);
    ...
    {
        bool parsed = PropertyFunctionParser.TryParse(input, MockElementLocation.Instance, out PropertyFunction expression);
    ...
    {
        PropertyFunction expression = Parse(input);
        Invocation first = expression.Invocations[0];
    ...
    public void ParsesAndNormalizesEveryArgumentForm(string argumentText, string?[] expectedArguments)
    {
        ...
        Invocation invocation = Parse(input).Invocations[0];
    ...
    [Theory]
    [MemberData(nameof(ArgumentFlagCases))]
    public void ClassifiesArgumentFlags(string argumentText, int expectedFlags)
    {
        Invocation invocation = Parse($"[Type]::Method({argumentText})").Invocations[0];

        invocation.Arguments.Count.ShouldBe(1);
        invocation.Arguments.GetFlags(0).ShouldBe((ArgumentFlags)expectedFlags);
    }

    [Fact]
    public void ClassifiesEachArgumentIndependently()
    {
        Invocation invocation =
            Parse("[Type]::Method(literal,$(Property),%3B,'$(Other)%20')").Invocations[0];

        invocation.Arguments.GetFlags(0).ShouldBe(ArgumentFlags.None);
        invocation.Arguments.GetFlags(1).ShouldBe(ArgumentFlags.ExpandProperties);
        invocation.Arguments.GetFlags(2).ShouldBe(ArgumentFlags.Unescape);
        invocation.Arguments.GetFlags(3).ShouldBe(ArgumentFlags.All);
    }
    ...
    {
        PropertyFunction expression = Parse(input);
        Invocation indexer = expression.Invocations[0];
    ...
    public void ParsesStaticMethodAndCompleteAccessChain()
    {
        ...
        foreach (Invocation invocation in expression.Invocations)
        {
    ...
    {
        const string expressionText = "[System.String]::Concat('$(A)', '%3B').Length";
        ...
        arguments[0].Value.ShouldBe("$(A)");
        arguments[0].Buffer.ShouldBeSameAs(buffer);
        arguments[0].Offset.ShouldBe(buffer.IndexOf("'$(A)'", StringComparison.Ordinal) + 1);
        arguments.GetFlags(0).ShouldBe(ArgumentFlags.ExpandProperties);
        arguments[1].Value.ShouldBe("%3B");
        arguments[1].Buffer.ShouldBeSameAs(buffer);
        arguments[1].Offset.ShouldBe(buffer.IndexOf("'%3B'", StringComparison.Ordinal) + 1);
        arguments.GetFlags(1).ShouldBe(ArgumentFlags.Unescape);
    }
    ...
    {
        if (!PropertyFunctionParser.TryParse(input, MockElementLocation.Instance, out PropertyFunction expression))
        {
            return -1;
        }
        ...
        foreach (Invocation invocation in expression)
        {
    ...
    private static PropertyFunction Parse(StringSegment input)
    {
        bool parsed = PropertyFunctionParser.TryParse(input, MockElementLocation.Instance, out PropertyFunction expression);
