...
public class Evaluator_Tests : IDisposable
{
    ...
    public void EnsureProjectEvaluationFinishedIsLogged()
    {
        ...
        using var collection = new ProjectCollection(new Dictionary<string, string>(), [logger], ToolsetDefinitionLocations.Default);
        _ = Assert.Throws<InvalidProjectFileException>(() => collection.LoadProject(projectFile.Path));
    ...
    public void VerifyLoadingImportScenarios(string importParameter, bool shouldSucceed)
    {
        ...
        try
        {
            var project = new Project(projectFile.Path);
            var logger = new MockLogger();
    ...
    public static TheoryData<string, bool> ImportLoadingScenarioTestData
    {
        get
        {
            const string NonExistentDirectory = "nonexistentDirectory";
            const string NonExistentFile = "projectThatDoesNotExist.csproj";
            const string RealDirectory = "realFolder";
            const string RealFile = "realFile.csproj";
            const string Glob = "*.*proj";
            const string VSToolsPath = "$(VSToolsPath)";

            string noFolderOrProject = Path.Combine(NonExistentDirectory, NonExistentFile);
            string folderButNoProject = Path.Combine(RealDirectory, NonExistentFile);
            string folderAndProject = Path.Combine(RealDirectory, RealFile);

            return new()
            {
                // This first section tests our behavior if a folder does not exist. Conditions and
                // whether there are wildcards should affect whether it fails if it fails to find a file.
                Success(projectPath: noFolderOrProject, condition: $"Exists('{noFolderOrProject}')"),
                Failure(projectPath: noFolderOrProject, condition: "'true'"),
                Failure(projectPath: noFolderOrProject),
                Success(projectPath: Path.Combine(NonExistentDirectory,Glob)),

                // This section tests if the folder does exist, but the project does not.
                Success(projectPath: folderButNoProject, condition: $"Exists('{folderButNoProject}')"),
                Failure(projectPath: folderButNoProject, condition: "'true'"),
                Failure(projectPath: folderButNoProject),
                Success(projectPath: Path.Combine(RealDirectory, Glob)),

                // This tests if the folder and the file exist.
                Success(projectPath: folderAndProject, condition: $"Exists('{folderAndProject}')"),
                Success(projectPath: folderAndProject, condition: "'true'"),
                Success(projectPath: folderAndProject),

                // If we fail to find a particular import along one project path, we have a few properties
                // that can be expanded in different ways, including VSToolsPath. In other words,
                // if the property isn't defined to somewhere that exists, we may still find it in a fallback
                // path. Error behavior in this case is more complicated, as the file may exist along one
                // search path but not another, in which case we should not error.
                Success(
                    projectPath: Path.Combine(VSToolsPath, noFolderOrProject),
                    condition: $"Exists('{Path.Combine(VSToolsPath, noFolderOrProject)}')"),
                Failure(
                    projectPath: Path.Combine(VSToolsPath, noFolderOrProject),
                    condition: "'true'"),
                Failure(projectPath: Path.Combine(VSToolsPath, noFolderOrProject)),
                Success(projectPath: Path.Combine(VSToolsPath, NonExistentDirectory, Glob)),
                Success(projectPath: Path.Combine(VSToolsPath, Glob)),
            };

            static (string, bool) Success(string projectPath, string? condition = null)
                => (ImportParameter(projectPath, condition), true);

            static (string, bool) Failure(string projectPath, string? condition = null)
                => (ImportParameter(projectPath, condition), false);

            static string ImportParameter(string projectPath, string? condition)
                => condition != null
                    ? $"""
                        Project="{projectPath}" Condition="{condition}"
                        """
                    : $"""
                        Project="{projectPath}"
                        """;
    ...
    public void VerifyExistsInMemoryProjects()
    {
        ...
        using var projectFromString = new ProjectFromString(projXml);
        ...
        using var projectFromStringFoo = new ProjectFromString(fooXml);
        ...
        using var projectFromStringBar = new ProjectFromString(barXml);
    ...
    public void VerifyConditionsInsideOutsideTargets()
    {
        ...
        const string TargetDirTargets2 = """
            <Project ToolsVersion=`msbuilddefaulttoolsversion` xmlns=`msbuildnamespace`>
        ...
        try
        {
            ...
            File.WriteAllText(textTextPath, TestText);
            ...
            var project = new Project(subdirProjPath);
    ...
    public void VerifyConditionsInsideOutsideTargets_ProjectInstance()
    {
        ...
        try
        {
            ...
            var project = new ProjectInstance(subdirProjPath);
    ...
    public void VerifyUsedUnInitializedPropertyInImports()
    {
        ...
        finally
        {
            ...
            FileUtilities.DeleteWithoutTrailingBackslash(targetDirectory, recursive: true);
        }
    }
    ...
    public void EmptyPropertyIsThenSet()
    {
        ...
        try
        {
            ...
            using var projectCollection = new ProjectCollection();
            projectCollection.RegisterLogger(logger);
            Project project = projectCollection.LoadProject(testTargetPath);
    ...
    {
        string importPath = FileUtilities.GetTemporaryFileName();
        ...
        try
        {
            File.WriteAllText(importPath, importContents);

            using var projectFromString = new ProjectFromString(projectContents);
            Project project = projectFromString.Project;
    ...
    {
        string importPath = FileUtilities.GetTemporaryFileName();
        string importPath2 = FileUtilities.GetTemporaryFileName();
        string importPath3 = FileUtilities.GetTemporaryFileName();

        string importContents1 = ObjectModelHelpers.CleanupFileContents("""
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
                    <p>Hello</p>
                </PropertyGroup>
        ...
        string importContents2 = ObjectModelHelpers.CleanupFileContents($"""
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
                    <p>Hello</p>
                </PropertyGroup>
        ...
        try
        {
            File.WriteAllText(importPath, importContents2);
            File.WriteAllText(importPath2, importContents2);
            File.WriteAllText(importPath3, importContents1);

            using var projectCollection = new ProjectCollection();
            using var projectFromString = new ProjectFromString(
                projectContents,
    ...
    public void ImportWithVSPathThrowsCorrectError()
    {
        ...
        string importPath = "path\\that\\does\\not\\exist\\Microsoft\\VisualStudio\\FileName.txt";

        TransientTestProjectWithFiles content = env.CreateTestProjectWithFiles($"""
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <Import Project='{importPath}'/>
            </Project>
            """);
        ...
        {
            _ = new Project(content.ProjectFile, globalProperties: null, toolsVersion: null);
        });
    ...
    {
        string importPath1 = FileUtilities.GetTemporaryFileName();
        string importPath2 = FileUtilities.GetTemporaryFileName();
        ...
        try
        {
            File.WriteAllText(importPath1, importContents1);
        ...
        try
        {
            ...
            _ = Assert.Throws<InvalidProjectFileException>(() =>
            {
                ...
                var project = new Project(
                    XmlReader.Create(new StringReader(manifest)),
                    globalProperties: null,
                    toolsVersion: null,
                    projectCollection,
    ...
    {
        string directory = Path.Combine(Path.GetTempPath(), "fol$der");
        string directory2 = Path.Combine(Path.GetTempPath(), $"fol$der{Path.DirectorySeparatorChar}fol$der2");
        ...
        string importPath1 = Path.Combine(directory, importRelative1);
        string importPath2 = Path.Combine(directory, importRelative2);
        string projectPath = Path.Combine(directory, "my.proj"); // project path has $ in too
        ...
        try
        {
            _ = Directory.CreateDirectory(directory2);
            ...
            _ = project.Xml.AddImport(importPathRelativeEscaped);
            Console.WriteLine(project.Xml.RawXml);
            var logger = new MockLogger();
            bool result = project.Build("t2", [logger]);
    ...
    public void ImportListOfItemsWithWildcards()
    {
        ...
        _ = ObjectModelHelpers.CreateFileInTempProjectDirectory($"{filename1}-1.props", """
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
            ...
            """);
        _ = ObjectModelHelpers.CreateFileInTempProjectDirectory($"{filename1}-2.props", """
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
            ...
            """);
        _ = ObjectModelHelpers.CreateFileInTempProjectDirectory($"{filename2}-1.props", """
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
            ...
            """);
        _ = ObjectModelHelpers.CreateFileInTempProjectDirectory($"{filename2}-2.props", """
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
            ...
            """);
        _ = ObjectModelHelpers.CreateFileInTempProjectDirectory($"{filename2}-3.props", """
            <Project ToolsVersion="msbuilddefaulttoolsversion" xmlns='msbuildnamespace' >
                <PropertyGroup>
    ...
    public void MSBuildThisFileProperties()
    {
        ...
        _ = main.AddImport(targets1FileName);
        _ = main.AddImport(targets2FileName);
        _ = main.AddImport(targets3FileName);
    ...
    public void PropertyPredecessors()
    {
        ...
        Assert.Same(xml1, property.Predecessor.Predecessor.Xml);
    ...
    public void PropertyPredecessorsAndImports()
    {
        ...
        Assert.Same(importXml, predecessorXmlRoot);
    }
    ...
    public void ItemDefinitionPredecessorToItemDefinition()
    {
        ...
        Assert.Same(xml1, metadatum.Predecessor.Predecessor.Xml);
    ...
    public void ItemDefinitionPredecessorToItem()
    {
        ...
        Assert.Same(xml2, metadatum.Predecessor.Xml);
    ...
    public void PredecessorOnSameItem()
    {
        ...
        Assert.Same(xml1, metadatum.Predecessor.Xml);
    ...
    public void ItemPredecessorToItemWithCaseChange()
    {
        ...
        ProjectItem item = Assert.Single(project.GetItems("i"));
    ...
    public void ItemPredecessorToItemWithCaseChangeAndEscapeHatch()
    {
        ...
        _ = env.SetEnvironmentVariable("MSBUILDUSECASESENSITIVEITEMNAMES", "1");
    ...
    public void ItemPredecessorsAndImports()
    {
        ...
        try
        {
            ...
            _ = import.AddItemDefinition("i").AddMetadata("m", "%(m);m1");
            import.Save(file);
            ...
            Assert.Same(import, predecessor.Xml.ContainingProject);
            Assert.Same(project.Xml, predecessor.Predecessor.Xml.ContainingProject);
        }
        finally
        {
            File.Delete(file);
    ...
    public void AllEvaluatedProperties()
    {
        ...
        var allEvaluatedPropertiesWithNoBackingXmlAndNoDuplicates = new Dictionary<string, ProjectProperty>(StringComparer.OrdinalIgnoreCase);
        ...
        {
            if (!allEvaluatedPropertiesWithNoBackingXmlAndNoDuplicates.TryGetValue(property.Name, out ProjectProperty? propertyFromAllEvaluated))
            {
                Assert.Fail($"project.Properties contained property {property.Name}, but AllEvaluatedProperties did not.");
            }
            else if (!property.Equals(propertyFromAllEvaluated))
            {
                Assert.Fail($"The properties in project.Properties and AllEvaluatedProperties for property {property.Name} were different.");
            }
        }
    ...
    public void AllEvaluatedItems()
    {
        ...
        try
        {
            ...
            _ = project.AddItem("i", "i6");
            _ = project.AddItem("i", "i7");
            _ = project.RemoveItem(project.AllEvaluatedItems.ElementAt(1));
    ...
    public void AllEvaluatedPropertiesAndImports()
    {
        ...
        try
        {
            ...
            _ = import.AddProperty("p", "1");
            _ = import.AddProperty("q", "2");
            import.Save();
    ...
    public void AllEvaluatedListsExceptPropertiesAreEmpty()
    {
        ...
        var allProjectPropertiesNoDuplicateNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (ProjectProperty property in project.AllEvaluatedProperties)
        {
            _ = allProjectPropertiesNoDuplicateNames.Add(property.Name);
        }
    ...
    public void ImportWildcardsRelative()
    {
        ...
        VerifyImportTargetRelativePath(directory, directory2, [Path.Combine("**", "*.targets")]);
    }
    ...
    public void ImportWildcardsRelative2()
    {
        ...
        VerifyImportTargetRelativePath(
            ...
            directory2,
            [
                Path.Combine(directory2, "*.targets"),
                Path.Combine(directory, "*.targets")
            ]);
    }
    ...
    public void ImportWildcardsRelative3()
    {
        ...
        VerifyImportTargetRelativePath(
            ...
            [
                Path.Combine(directory2, "..", "*.targets"),
                Path.Combine(directory, ".", "sub", "*.targets")
            ]);
    }
    ...
    public void ImportWildcardsFullPath()
    {
        ...
        VerifyImportTargetRelativePath(directory, directory2, [file1, file2, file3]);
    }
    ...
    public void MSBuildExtensionsPathWithGlobalOverride()
    {
        ...
        // of seeing whether our value wins.
        Assert.True(project.SetGlobalProperty("MSBuildExtensionsPath", @"c:\devdiv\vscore\msbuild"));
        project.ReevaluateIfNecessary();
    ...
    public void MSBuildExtensionsPath32WithGlobalOverride()
    {
        ...
        Assert.True(project.SetGlobalProperty("MSBuildExtensionsPath32", @"c:\devdiv\vscore\msbuild"));
        string msbuildExtensionsPath32Value = project.GetPropertyValue("MSBuildExtensionsPath32");
        Assert.Equal(@"c:\devdiv\vscore\msbuild", msbuildExtensionsPath32Value);
    }
    ...
    public void MSBuildExtensionsPath64WithGlobalOverride()
    {
        ...
        Assert.True(project.SetGlobalProperty("MSBuildExtensionsPath64", @"c:\devdiv\vscore\msbuild"));
        string msbuildExtensionsPath64Value = project.GetPropertyValue("MSBuildExtensionsPath64");
        Assert.Equal(@"c:\devdiv\vscore\msbuild", msbuildExtensionsPath64Value);
    }
    ...
    public void LocalAppDataWithGlobalOverride()
    {
        ...
        Assert.True(project.SetGlobalProperty("LocalAppData", @"c:\AppData\Local"));
        string localAppDataValue = project.GetPropertyValue("LocalAppData");
        Assert.Equal(@"c:\AppData\Local", localAppDataValue);
    }
    ...
    public void MSBuildVersion()
    {
        ...
        msbuildVersionAsVersion.Minor.ShouldBeInRange(
            from: 0,
            to: 20,
        ...
        ProjectCollection.Version.ToString().ShouldStartWith(
            expected: msbuildVersionProperty,
    ...
    public void VerifyMsbuildProgramFiles32ReservedProperty()
    {
        ...
        bool result = project.Build(logger);
        Assert.True(result);
        logger.AssertLogContains($"[{FrameworkLocationHelper.programFiles32}]");
    }
    ...
    public void VerifyTreatAsLocalPropertyTrimmed()
    {
        ...
        using var projectFromString = new ProjectFromString(
            content,
            globalProperties: (IDictionary<string, string>)new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                { "Foo", "Baz" },
                { "Goo", "Foo" }
            },
    ...
    public void VerifyModificationsToGlobalPropertiesRespected()
    {
        ...
        Assert.True(project.SetGlobalProperty("Foo", "Baz"));
        project.ReevaluateIfNecessary();
    ...
    public void VerifyDefaultSubToolsetPropertiesAreEvaluated()
    {
        ...
        try
        {
            ...
            using ProjectCollection fakeProjectCollection = GetProjectCollectionWithFakeToolset(globalProperties: null);
            using var projectFromString = new ProjectFromString(content, globalProperties: null, toolsVersion: "Fake", fakeProjectCollection);
            Project project = projectFromString.Project;
    ...
    public void VerifyExplicitNonExistentSubToolsetPropertiesAreEvaluated()
    {
        ...
        try
        {
            ...
            using ProjectCollection fakeProjectCollection = GetProjectCollectionWithFakeToolset(globalProperties: null);
            using var projectFromString = new ProjectFromString(content, globalProperties: null, "Fake", fakeProjectCollection);
            Project project = projectFromString.Project;
    ...
    public void VerifySubToolsetVersionSetByConstructorOverridable_OverridesGlobalProperty()
    {
        ...
        using var projectFromString = new ProjectFromString(
            ...
            toolsVersion: "Fake",
            subToolsetVersion: "FakeSubToolset",
            fakeProjectCollection,
    ...
    public void VerifyVisualStudioVersionSetByToolset()
    {
        ...
        try
        {
            ...
            using var collection = new ProjectCollection();
            var project = new Project(globalProperties: null, ObjectModelHelpers.MSBuildDefaultToolsVersion, collection);
    ...
    public async Task VerifyDTDProcessingIsDisabled2()
    {
        ...
        var t = new Thread(HttpServerThread)
        {
            IsBackground = true
        };
    ...
    public void VerifyConditionEvaluatorResetStateOnFailure()
    {
        ...
        var expander = new Expander<ProjectPropertyInstance, ProjectItemInstance>(propertyBag, FileSystems.Default);
        const string Condition = " '$(TargetOSFamily)' >= '3' ";
        ...
        try
        {
            _ = ConditionEvaluator.EvaluateCondition(
                Condition,
    ...
    public void ThrownInvalidProjectExceptionProperlyHandled()
    {
        ...
        try
        {
            ...
            InvalidProjectFileException ex = Assert.Throws<InvalidProjectFileException>(() =>
            {
                _ = new Project(primaryProject, globalProperties: null, toolsVersion: null);
            });
    ...
    public void ConditionWithShortCircuitAndErrorDoesNotFailBuild(string projectInnerXml)
    {
        ...
        _ = env.SetEnvironmentVariable("MSBUILDLOGIMPORTS", "1");
        using var projectFromString = new ProjectFromString(content);
        Project project = projectFromString.Project;
    ...
    public void VerifyMSBuildLastModifiedProjectForImport()
    {
        ...
        TransientTestProjectWithFiles project1 = testEnvironment.CreateTestProjectWithFiles("<Project />");
        TransientTestProjectWithFiles project2 = testEnvironment.CreateTestProjectWithFiles("<Project />");

        TransientTestProjectWithFiles primaryProject = testEnvironment.CreateTestProjectWithFiles($"""
            <Project>
        ...
        var project = new Project(primaryProject.ProjectFile, globalProperties: null, toolsVersion: null);
    ...
    public void VerifyMSBuildLogsAMessageWhenLocalPropertyCannotOverrideValueOfGlobalProperty()
    {
        ...
        using var projectFromString = new ProjectFromString(content, globalProperties, toolsVersion: null, collection);
    ...
    public void VerifyPropertyTrackingLoggingPropertyInitialValue()
        => VerifyPropertyTrackingLoggingScenario(
            ...
            (logger, projectPath) =>
            {
                ...
                var propertyInitialValueMap = logger
                    .AllBuildEvents
                    .OfType<PropertyInitialValueSetEventArgs>()
    ...
    public void VerifyGetTypeEvaluationBlocked()
    {
        ...
        _ = Should.Throw<InvalidProjectFileException>(() =>
            new Project(
    ...
    private static void VerifyPropertyTrackingLoggingScenario(string? envVarValue, Action<MockLogger, string> loggerEvaluatorAction)
    {
        ...
        {
            _ = env.SetEnvironmentVariable("MsBuildLogPropertyTracking", envVarValue);
        }

        _ = env.SetEnvironmentVariable("DEFINED_ENVIRONMENT_VARIABLE", "It's Defined!");
        _ = env.SetEnvironmentVariable("DEFINED_ENVIRONMENT_VARIABLE2", "It's also Defined!");

        TransientTestFile tempPath = env.CreateFile(Guid.NewGuid().ToString(), testTargets);
        ...
        using var projectCollection = new ProjectCollection();
        projectCollection.RegisterLogger(logger);
        Project project = projectCollection.LoadProject(tempPath.Path);
    ...
    public void VerifyLogPropertyReassignment()
    {
        ...
        _ = env.CreateFolder(targetDirectory);
        _ = env.CreateFile(testTargetPath, testtargets);
    ...
    private ProjectCollection GetProjectCollectionWithFakeToolset(IDictionary<string, string>? globalProperties)
    {
        ...
        var subToolsets = new Dictionary<string, SubToolset>(StringComparer.OrdinalIgnoreCase);

        // SubToolset 11.0 properties
        var subToolset11Properties = new PropertyDictionary<ProjectPropertyInstance>();
        ...
        var fakeSubToolsetProperties = new PropertyDictionary<ProjectPropertyInstance>();
        ...
        var fakeToolset = new Toolset("Fake", parentToolset.ToolsPath, properties, projectCollection, subToolsets, parentToolset.OverrideTasksPath);
    ...
    private void VerifyImportTargetRelativePath(string directory, string directory2, string[] imports)
    {
        ...
        string file2 = Path.Combine(directory2, "2.targets");
        string file3 = Path.Combine(directory2, "3.cpp.targets");
        string file4 = Path.Combine(directory2, "4.nottargets");
        ...
        try
        {
            ...
            logger.AssertLogContains(["[2.targets]", "[3.cpp.targets]", "[1.targets]"]);
        ...
        finally
        {
            ...
            File.Delete(file3);
            File.Delete(file4);
            FileUtilities.DeleteWithoutTrailingBackslash(directory, recursive: true);
        }
    }
}
