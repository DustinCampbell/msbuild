...
public sealed class ExpanderFactory_Tests(ITestOutputHelper output) : IDisposable
{
    ...
    {
        using TestEnvironment env = TestEnvironment.Create(output);
        SetSelectionEnvironment(env, ChangeWaves.Wave18_13.ToString(), useLegacyExpander: null);

        CreateExpander().ShouldBeOfType<LegacyExpander<ProjectPropertyInstance, ProjectItemInstance>>();
        ChangeWaves.DisabledWave.ShouldBe(ChangeWaves.Wave18_13);
        ChangeWaves.ConversionState.ShouldBe(ChangeWaveConversionState.Valid);
        ChangeWaves.AreFeaturesEnabled(ChangeWaves.Wave18_12).ShouldBeTrue();
    }
