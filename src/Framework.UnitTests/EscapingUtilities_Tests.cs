...
public sealed class EscapingUtilities_Tests
{
    ...
    public void UnescapeAll_ShouldDecodeEscapeSequences(string input, string expected)
    ...
    [InlineData("%*?*%*", "%25%2a%3f%2a%25%2a")]
    public void Escape_ShouldEncodeSpecialCharacters(string input, string expected)
        => EscapingUtilities.Escape(input).ShouldBe(expected);

    [Theory]
    [InlineData("*")]
    [InlineData("?")]
    [InlineData("#*?*#*")]
    public void UnescapeAll_AfterEscape_ShouldRoundTrip(string text)
    ...
    [InlineData("%3f")]
    [InlineData("#%2a%3f%2a#%2a")]
    public void Escape_AfterUnescapeAll_ShouldRoundTrip(string text)
    ...
    public void ContainsEscapedWildcards_ShouldDetectEscapedWildcards(string input, bool expected)
        => EscapingUtilities.ContainsEscapedWildcards(input).ShouldBe(expected);
}
