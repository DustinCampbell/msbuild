    ...
    IComparable<string?>
{
#if !NET
    /// <summary>
    ///  The invariant <see cref="TextInfo"/>, cached to avoid repeatedly walking the
    ///  <see cref="CultureInfo.InvariantCulture"/> and <see cref="CultureInfo.TextInfo"/> property getters
    ///  that <see cref="char.ToUpperInvariant(char)"/> goes through on each call. The invariant
    ///  <see cref="TextInfo"/> is read-only, so caching it is safe.
    /// </summary>
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private StringSegment(StringSegment source, int start)
    {
        if ((uint)start > (uint)source.Length)
        {
            ThrowInvalidRange(start, length: 0, source.Length);
        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private StringSegment(StringSegment source, int start, int length)
    {
        if ((uint)start > (uint)source.Length || (uint)length > (uint)(source.Length - start))
        {
            ThrowInvalidRange(start, length, source.Length);
        }
    ...
    private static void ThrowInvalidArguments(string? buffer, int offset, int length)
    {
        ...
        ThrowInvalidRange(offset, length, buffer.Length);
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    private static void ThrowInvalidIndex(int index, int length)
    {
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static void ThrowInvalidRange(int start, int length, int segmentLength)
    {
...
#if NET
        return Ascii.IsValid(AsSpan());
#else
        // OR every character together; a set bit at or above 0x80 means a non-ASCII character was present.
        int remaining = Length;
        int accumulator = 0;
    ...
    public char this[int index]
    {
        ...
        get
        {
            ...
            {
                ThrowInvalidIndex(index, Length);
            }
    ...
    public ReadOnlySpan<char> AsSpan(int start)
    {
        ...
        {
            ThrowInvalidRange(start, length: 0, Length);
        }
    ...
    public ReadOnlySpan<char> AsSpan(int start, int length)
    {
        ...
        {
            ThrowInvalidRange(start, length, Length);
        }
    ...
    /// <param name="comparisonType">One of the enumeration values that specifies how the values are compared.</param>
    /// <returns>
    ///  <see langword="true"/> if this segment begins with <paramref name="value"/>; otherwise, <see langword="false"/>.
    /// </returns>
    public bool StartsWith(StringSegment value, StringComparison comparisonType = StringComparison.Ordinal)
    {
        ...
        // A prefix match is an equality comparison of this segment's leading value.Length characters against
        // value. Comparing the two regions directly avoids allocating a needle string on .NET Framework.
        return value.Length == 0
            || CompareCore(Buffer!, Offset, value.Length, value.Buffer!, value.Offset, value.Length, comparisonType) == 0;
    }
    ...
    /// <param name="comparisonType">One of the enumeration values that specifies how the values are compared.</param>
    /// <returns>
    ///  <see langword="true"/> if this segment ends with <paramref name="value"/>; otherwise, <see langword="false"/>.
    /// </returns>
    public bool EndsWith(StringSegment value, StringComparison comparisonType = StringComparison.Ordinal)
    {
        ...
        // A suffix match is an equality comparison of this segment's trailing value.Length characters against
        // value. Comparing the two regions directly avoids allocating a needle string on .NET Framework.
        return value.Length == 0
            || CompareCore(Buffer!, Offset + Length - value.Length, value.Length, value.Buffer!, value.Offset, value.Length, comparisonType) == 0;
    }
