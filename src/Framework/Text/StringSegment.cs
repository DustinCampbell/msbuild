    ...
    ISpanFormattable
{
    ...
    {
        Assumed.PositiveOrZero(startIndex);
        Assumed.PositiveOrZero(length);
        Assumed.True(startIndex <= value.Length - length || (startIndex == 0 && length == 0));
    ...
    public char this[int index]
        => _value?[_startIndex + index] ?? string.Empty[index];
    ...
    /// <returns>
    ///  A new <see cref="StringSegment"/> that starts at the specified index.
    /// </returns>
    public StringSegment Slice(int startIndex)
    {
        Assumed.LessThanOrEqual((uint)startIndex, (uint)_length);

        return new(_startIndex + startIndex, _length - startIndex, _value ?? string.Empty);
    }
    ...
    {
        Assumed.LessThanOrEqual((uint)startIndex, (uint)_length);
        Assumed.LessThanOrEqual((uint)length, (uint)(_length - startIndex));

        return new(_startIndex + startIndex, length, _value ?? string.Empty);
    }

    /// <summary>
    ///  Splits on the next separator, or returns the entire segment if no separator is found.
    /// </summary>
    /// <param name="separator">A character that delimits the segments.</param>
    ...
    public bool TrySplit(char separator, out StringSegment left, out StringSegment right)
    {
        if (_length == 0)
        {
            left = default;
    ...
    /// </summary>
    public int IndexOfAny(char value0, char value1) // Optimizing for DirectorySeparatorChar == AltDirectorySeparatorChar
        => value0 == value1
            ? IndexOf(value0)
    ...
    /// </summary>
    public int LastIndexOfAny(char value0, char value1) // Optimizing for DirectorySeparatorChar == AltDirectorySeparatorChar
        => value0 == value1
            ? LastIndexOf(value0)
    ...
    public StringSegment Trim()
    {
        ...
        return new(start, end - start + 1, _value);
    }
    ...
    public StringSegment TrimStart()
    {
        ...
        return new(start, _length - (start - _startIndex), _value);
    }
    ...
    public StringSegment TrimEnd()
    {
        ...
        return new(_startIndex, end - _startIndex + 1, _value);
    }
    ...
    /// </summary>
    /// <returns>
    ///  A span that represents the segment.
    /// </returns>
    public ReadOnlySpan<char> AsSpan()
        => _value.AsSpan(_startIndex, _length);
    ...
    /// <returns>
    ///  A span that represents the specified portion of the segment.
    /// </returns>
    public ReadOnlySpan<char> AsSpan(int start)
    {
        Assumed.LessThanOrEqual((uint)start, (uint)_length);

        return _value.AsSpan(_startIndex + start, _length - start);
    }
    ...
    {
        Assumed.LessThanOrEqual((uint)start, (uint)_length);
        Assumed.LessThanOrEqual((uint)length, (uint)(_length - start));

        return _value.AsSpan(_startIndex + start, length);
    }
    ...
    /// </summary>
    /// <returns>
    ///  A <see langword="string"/> that represents the current segment.
    /// </returns>
    public override string ToString()
        => _length == 0 || _value is null
            ? string.Empty
            : _startIndex switch
            {
    ...
    public bool Equals(StringSegment other, StringComparison comparison)
        => _length == other.Length // If they're both empty, there is nothing to do.
            && (_length == 0
    ...
    public int CompareTo(StringSegment other, StringComparison comparison)
        => comparison switch
        {
            StringComparison.Ordinal
                => AsSpan().CompareOrdinalAsString(other),
            StringComparison.OrdinalIgnoreCase
                => CompareToOrdinalIgnoreCase(other.Value, other._startIndex, other._length),
            ...
            _ => Assumed.Unreachable<int>("Unsupported comparison type."),
        };
    ...
    public int CompareTo(string other, StringComparison comparison)
        => comparison switch
        {
            StringComparison.Ordinal
                => AsSpan().CompareOrdinalAsString(other),
            StringComparison.OrdinalIgnoreCase
                => CompareToOrdinalIgnoreCase(other, 0, other.Length),
            StringComparison.CurrentCulture
    ...
    /// </returns>
    private static unsafe bool TryCompareAsciiIgnoreCase(char* a, int lengthA, char* b, int lengthB, out int scanned, out int result)
    {
