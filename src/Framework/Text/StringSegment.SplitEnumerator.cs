...
{
    /// <summary>
    ///  Enumerates the pieces produced by <see cref="Split(char, StringSplitOptions)"/> and its overloads.
    ...
    private const StringSplitOptions TrimEntriesOption = StringSplitOptions.TrimEntries;
        ...
        public SplitEnumerator(StringSegment segment, char separator, StringSplitOptions options)
        {
            _segment = segment;
            _separators = default;
        ...
        /// <param name="options">Options that control empty-entry removal and entry trimming.</param>
        public SplitEnumerator(StringSegment segment, ReadOnlySpan<char> separators, StringSplitOptions options)
        {
            _segment = segment;
        ...
        private readonly int FindSeparator(int start)
            => _hasSingleSeparator
                ? _segment.IndexOf(_separator, start)
                : _segment.IndexOfAny(_separators, start);
    }
}
