...
internal static class StringBuilderCache
{
    ...
    public static StringBuilder Acquire(int capacity = 16 /*StringBuilder.DefaultCapacity*/)
    {
        ...
        if (capacity <= MAX_BUILDER_SIZE)
        {
            ...
            if (builder != null)
            {
                ...
                {
                    builder.Length = 0; // Equivalent of sb.Clear() that works on .Net 3.5
#if DEBUG && !CLR2COMPATIBILITY && !MICROSOFT_BUILD_ENGINE_OM_UNITTESTS
                    MSBuildEventSource.Log.ReusableStringBuilderFactoryStart(hash: builder.GetHashCode(), newCapacity: capacity, oldCapacity: builder.Capacity, type: "sbc-hit");
#endif
                    return builder;
                }
            }
...
#if DEBUG && !CLR2COMPATIBILITY && !MICROSOFT_BUILD_ENGINE_OM_UNITTESTS
        MSBuildEventSource.Log.ReusableStringBuilderFactoryStart(hash: builder.GetHashCode(), newCapacity: capacity, oldCapacity: builder.Capacity, type: "sbc-miss");
#endif
        return builder;
    }
    ...
    /// </summary>
    /// <param name="builder">The <see cref="StringBuilder"/> to cache. Likely returned from <see cref="Acquire(int)"/>.</param>
    /// <remarks>
...
#if DEBUG && !CLR2COMPATIBILITY && !MICROSOFT_BUILD_ENGINE_OM_UNITTESTS
        MSBuildEventSource.Log.ReusableStringBuilderFactoryStop(hash: builder.GetHashCode(), returningCapacity: builder.Capacity, returningLength: builder.Length, type: builder.Capacity <= MAX_BUILDER_SIZE ? "sbc-return" : "sbc-discard");
#endif
    }
