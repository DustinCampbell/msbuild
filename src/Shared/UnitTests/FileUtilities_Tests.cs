...
namespace Microsoft.Build.UnitTests
{
    public class FileUtilities_Tests
    {
        ...
        {
            string cache = null;
            string modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.RecursiveDir, "foo", currentDirectory, String.Empty, ref cache);
            Assert.Equal(String.Empty, modifier);

            cache = null;
            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.ModifiedTime, "foo", currentDirectory, String.Empty, ref cache);
            Assert.Equal(String.Empty, modifier);

            cache = null;
            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.RelativeDir, @"foo\goo", currentDirectory, String.Empty, ref cache);
            ...
            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.FullPath, itemSpec, currentDirectory, String.Empty, ref cache);
            Assert.Equal(itemSpec, modifier);
            Assert.Equal(itemSpec, cache);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.RootDir, itemSpec, currentDirectory, String.Empty, ref cache);
            Assert.Equal(itemSpecDir, modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.Filename, itemSpec, currentDirectory, String.Empty, ref cache);
            Assert.Equal(@"foo", modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.Extension, itemSpec, currentDirectory, String.Empty, ref cache);
            Assert.Equal(@".txt", modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.Directory, itemSpec, currentDirectory, String.Empty, ref cache);
            Assert.Equal(String.Empty, modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.Identity, itemSpec, currentDirectory, String.Empty, ref cache);
            Assert.Equal(itemSpec, modifier);
            ...
            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.DefiningProjectDirectory, itemSpec, currentDirectory, projectPath, ref cache);
            Assert.Equal(projectPathDir, modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.DefiningProjectExtension, itemSpec, currentDirectory, projectPath, ref cache);
            Assert.Equal(@".proj", modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.DefiningProjectFullPath, itemSpec, currentDirectory, projectPath, ref cache);
            Assert.Equal(projectPath, modifier);

            modifier = ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.DefiningProjectName, itemSpec, currentDirectory, projectPath, ref cache);
        ...
        private static void TestGetItemSpecModifierOnBadPath(string currentDirectory)
        {
            ...
            {
                string cache = null;
                ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.RootDir, @"http://www.microsoft.com", currentDirectory, String.Empty, ref cache);
            }
            catch (Exception e)
            {
        ...
        public void GetItemSpecModifierRootDirThatFitsIntoMaxPath()
        {
            ...
            Assert.Equal(@"c:\", ItemSpecModifiers.GetItemSpecModifier(ItemSpecModifiers.RootDir, fullPath, currentDirectory, String.Empty, ref cache));
        }
