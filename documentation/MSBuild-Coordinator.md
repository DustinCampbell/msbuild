...
**Server → Client:**
- `NodeGrantMessage` - Grants nodes to a build. A single C# type covering two wire shapes: the legacy int-only payload, and (when both sides support `nested-grants`) a payload that also carries a root grant token (`GrantId`). Both shapes use the same `NodeGrant` message type byte. When `GrantId` is present, the type byte includes the extended-fields marker bit and is followed by a single extended-fields byte whose `GrantId` flag indicates the token is included in the payload.
...
1. The root build receives a grant token in the `GrantId` field of `NodeGrantMessage`.
