...
| Supply environmental state during expansion | `Expander<P, I>.ExpansionContext` | [Expander.ExpansionContext.cs](../../src/Build/Evaluation/Expander.ExpansionContext.cs) |
...
`Invocation` in order, carrying each `functionResult` as the **live object** receiver for the next
...
3. **Late bind** - on `MissingMethodException`, `PropertyFunctionInvoker.LateBind` tries an all-`string`
   signature, then matches by name + argument count and runs `PropertyFunctionInvoker.CoerceArguments`.

`PropertyFunctionInvoker.CoerceArguments` is the explicit conversion table:
