...
| File:Line | IL rule | Member | Triggering call | Justification (short) | Status |
| --- | --- | --- | --- | --- | --- |
| [Expander.FunctionBinder.cs:23](../../src/Build/Evaluation/Expander.FunctionBinder.cs#L23) | IL2067 | `FunctionBinder.Bind(...)` | pass resolved receiver type to the DAM-annotated `Function` constructor | Static receiver types come from `AvailableStaticMembers`; runtime receiver types are restricted under trimming. Both sets have their public members preserved. | Vetted |
| [Expander.FunctionBinder.cs:27](../../src/Build/Evaluation/Expander.FunctionBinder.cs#L27) | IL2072 | `FunctionBinder.Bind(...)` | runtime receiver `GetType()` flows to the DAM-annotated `Function` constructor | `RestrictPropertyFunctionReceivers` is substituted `true` under trimming, bounding runtime receiver types to the preserved `PropertyFunctionReceiver` allowlist. | Vetted |
| [Expander.Function.cs:215](../../src/Build/Evaluation/Expander.Function.cs#L215) | IL2074 | `Function.Execute(...)` | `_receiverType` from runtime value | Same bounded-allowlist receiver as above; `AvailableStaticMembers.PropertyFunctionMembers` preserves the reflected members under trim (the code comment there calls these suppressions "honest under trimming"). | Vetted |
| [Expander.Function.cs:219](../../src/Build/Evaluation/Expander.Function.cs#L219) | IL2080 | `Function.Execute(...)` | `_receiverType.GetMethods(_bindingFlags)` (out-param path) | `_bindingFlags` is masked to `AllowedBindingFlags` at construction so it never carries `BindingFlags.NonPublic`; the call binds only public methods of the bounded allowlist receiver, whose public members are preserved for trimming. | Vetted |
...
| [Expander.Function.cs:724](../../src/Build/Evaluation/Expander.Function.cs#L724) | IL2080 | `Function.FindPublicMethodBySignature(...)` | `_receiverType.GetMethods(_bindingFlags)` | Same `_bindingFlags` no-`NonPublic` invariant; public-only bind over the bounded allowlist receiver (the GetMethods signature match added with #14191). | Vetted |
| [Expander.Function.cs:777](../../src/Build/Evaluation/Expander.Function.cs#L777) | IL3050 | `Function.LateBindExecute(...)` | `Enum.GetValues(Type)` (rooted) | `Enum.GetValues(Type)` is unreachable via property functions - no way to supply a `Type` arg (MSB4185/MSB4186) - and would fail observably if reached; proven under Native AOT by `PropertyFunctionAotTests`. | Vetted |
| [Expander.Function.cs:781](../../src/Build/Evaluation/Expander.Function.cs#L781) | IL2080 | `Function.LateBindExecute(...)` | `_receiverType.GetMethods(_bindingFlags)` | Same `_bindingFlags` no-`NonPublic` invariant; public-only bind over the bounded allowlist receiver. | Vetted |
