...
| File:Line | IL rule | Member | Triggering call | Justification (short) | Status |
| --- | --- | --- | --- | --- | --- |
| [Expander.FunctionBuilder.cs:55](../../src/Build/Evaluation/Expander.FunctionBuilder.cs#L55) | IL2069 | `FunctionBuilder.SetReceiverType(Type)` | store into DAM-annotated `ReceiverType` | Receiver bounded to preserved-member allowlists: static types to `AvailableStaticMembers` (members preserved by `AvailableStaticMembers.PropertyFunctionMembers` `[DynamicDependency]`); instance receivers to `PropertyFunctionReceiver` (§10 `RestrictPropertyFunctionReceivers` substituted `true` under trim). The reflected members are preserved, so the dataflow warning is a false positive. | Vetted |
| [Expander.Function.cs:363](../../src/Build/Evaluation/Expander.Function.cs#L363) | IL2074 | `Function.Execute(...)` | `_receiverType` from runtime value | Same bounded-allowlist receiver as above; `AvailableStaticMembers.PropertyFunctionMembers` preserves the reflected members under trim (the code comment there calls these suppressions "honest under trimming"). | Vetted |
| [PropertyFunctionInvoker.cs:90](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs#L90) | IL2070 | `PropertyFunctionInvoker.TryBindAndInvokeMethodWithOutArguments(...)` | `receiverType.GetMethods(bindingFlags)` | `InvokeMember` rejects `BindingFlags.NonPublic` before reaching this private helper; the receiver parameter preserves the public property-function member surface. | Vetted |
| [AvailableStaticMembers.cs:111](../../src/Build/Evaluation/Expander/AvailableStaticMembers.cs#L111) | IL2096 | `AvailableStaticMembers.ResolveTypeByName(...)` | case-insensitive lookup of a property-function receiver type | Resolution is constrained by the preserved allowlist and the subsequent static-member availability gate. | Vetted |
| [PropertyFunctionInvoker.cs:233](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs#L233) | IL2070 | `PropertyFunctionInvoker.FindPublicMethodBySignature(...)` | `receiverType.GetMethods(bindingFlags)` | Same public-only flags invariant and annotated receiver parameter. | Vetted |
| [PropertyFunctionInvoker.cs:281](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs#L281) | IL3050 | `PropertyFunctionInvoker.LateBind(...)` | `Enum.GetValues(Type)` (rooted) | `Enum.GetValues(Type)` is unreachable via property functions - no way to supply a `Type` arg (MSB4185/MSB4186) - and would fail observably if reached; proven under Native AOT by `PropertyFunctionAotTests`. | Vetted |
| [PropertyFunctionInvoker.cs:281](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs#L281) | IL2070 | `PropertyFunctionInvoker.LateBind(...)` | `receiverType.GetMethods(bindingFlags)` | Same public-only flags invariant and annotated receiver parameter. | Vetted |
| [AvailableStaticMembers.cs:252](../../src/Build/Evaluation/Expander/AvailableStaticMembers.cs#L252) | IL3050 | `AvailableStaticMembers.CreateAvailableMembers()` | `Enum.GetValues(Type)` (rooted for allowlist) | Same `Enum.GetValues(Type)` false positive: rooted by `typeof(Enum)` for the allowlist but unreachable via property functions; verified by `PropertyFunctionAotTests`. | Vetted |
