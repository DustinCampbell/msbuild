...
| File:Line | IL rule | Member | Triggering call | Justification (short) | Status |
| --- | --- | --- | --- | --- | --- |
| [Expander.FunctionBinder.cs:25](../../src/Build/Evaluation/Expander.FunctionBinder.cs#L25) | IL2067 | `FunctionBinder.Bind(...)` | pass resolved receiver type to the DAM-annotated `Function` constructor | Static receiver types come from `AvailableStaticMembers`; runtime receiver types are restricted under trimming. Both sets have their public members preserved. | Vetted |
| [Expander.FunctionBinder.cs:29](../../src/Build/Evaluation/Expander.FunctionBinder.cs#L29) | IL2072 | `FunctionBinder.Bind(...)` | runtime receiver `GetType()` flows to the DAM-annotated `Function` constructor | `RestrictPropertyFunctionReceivers` is substituted `true` under trimming, bounding runtime receiver types to the preserved `PropertyFunctionReceiver` allowlist. | Vetted |
...
| [Expander.Function.cs:712](../../src/Build/Evaluation/Expander.Function.cs#L712) | IL2080 | `Function.FindPublicMethodBySignature(...)` | `_receiverType.GetMethods(_bindingFlags)` | Same `_bindingFlags` no-`NonPublic` invariant; public-only bind over the bounded allowlist receiver (the GetMethods signature match added with #14191). | Vetted |
| [Expander.Function.cs:765](../../src/Build/Evaluation/Expander.Function.cs#L765) | IL3050 | `Function.LateBindExecute(...)` | `Enum.GetValues(Type)` (rooted) | `Enum.GetValues(Type)` is unreachable via property functions - no way to supply a `Type` arg (MSB4185/MSB4186) - and would fail observably if reached; proven under Native AOT by `PropertyFunctionAotTests`. | Vetted |
| [Expander.Function.cs:769](../../src/Build/Evaluation/Expander.Function.cs#L769) | IL2080 | `Function.LateBindExecute(...)` | `_receiverType.GetMethods(_bindingFlags)` | Same `_bindingFlags` no-`NonPublic` invariant; public-only bind over the bounded allowlist receiver. | Vetted |
