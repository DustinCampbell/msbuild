...
| File:Line | IL rule | Member | Triggering call | Justification (short) | Status |
| --- | --- | --- | --- | --- | --- |
| [Expander.PropertyFunctionExecutor.Binding.cs](../../src/Build/Evaluation/Expander.PropertyFunctionExecutor.Binding.cs) | IL2067 | `PropertyFunctionExecutor.Bind(...)` | pass resolved receiver type to the DAM-annotated `BoundFunction` constructor | Static receiver types come from `AvailableStaticMembers`; runtime receiver types are restricted under trimming. Both sets have their public members preserved. | Vetted |
| [Expander.PropertyFunctionExecutor.Binding.cs](../../src/Build/Evaluation/Expander.PropertyFunctionExecutor.Binding.cs) | IL2072 | `PropertyFunctionExecutor.Bind(...)` | runtime receiver `GetType()` flows to the DAM-annotated `BoundFunction` constructor | `RestrictPropertyFunctionReceivers` is substituted `true` under trimming, bounding runtime receiver types to the preserved `PropertyFunctionReceiver` allowlist. | Vetted |
| [Expander.PropertyFunctionExecutor.cs](../../src/Build/Evaluation/Expander.PropertyFunctionExecutor.cs) | IL2074 | `BoundFunction.Execute(...)` | `_receiverType` from runtime value | Same bounded-allowlist receiver as above; `AvailableStaticMembers.PropertyFunctionMembers` preserves the reflected members under trim. | Vetted |
| [PropertyFunctionInvoker.cs](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs) | IL2070 | `PropertyFunctionInvoker.TryBindAndInvokeMethodWithOutArguments(...)` | `receiverType.GetMethods(bindingFlags)` | `InvokeMember` rejects `BindingFlags.NonPublic` before reaching this private helper; the receiver parameter preserves the public property-function member surface. | Vetted |
| [AvailableStaticMembers.cs:84](../../src/Build/Evaluation/Expander/AvailableStaticMembers.cs#L84) | IL2096 | `AvailableStaticMembers.ResolveTypeByName(...)` | case-insensitive lookup of a property-function receiver type | Resolution is constrained by the preserved allowlist and the subsequent static-member availability gate. | Vetted |
| [PropertyFunctionInvoker.cs](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs) | IL2070 | `PropertyFunctionInvoker.FindPublicMethodBySignature(...)` | `receiverType.GetMethods(bindingFlags)` | Same public-only flags invariant and annotated receiver parameter. | Vetted |
| [PropertyFunctionInvoker.cs](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs) | IL3050 | `PropertyFunctionInvoker.LateBind(...)` | `Enum.GetValues(Type)` (rooted) | `Enum.GetValues(Type)` is unreachable via property functions - no way to supply a `Type` arg (MSB4185/MSB4186) - and would fail observably if reached; proven under Native AOT by `PropertyFunctionAotTests`. | Vetted |
| [PropertyFunctionInvoker.cs](../../src/Build/Evaluation/Expander/PropertyFunctionInvoker.cs) | IL2070 | `PropertyFunctionInvoker.LateBind(...)` | `receiverType.GetMethods(bindingFlags)` | Same public-only flags invariant and annotated receiver parameter. | Vetted |
| [AvailableStaticMembers.cs:246](../../src/Build/Evaluation/Expander/AvailableStaticMembers.cs#L246) | IL3050 | `AvailableStaticMembers.CreateAvailableMembers()` | `Enum.GetValues(Type)` (rooted for allowlist) | Same `Enum.GetValues(Type)` false positive: rooted by `typeof(Enum)` for the allowlist but unreachable via property functions; verified by `PropertyFunctionAotTests`. | Vetted |
...
> preserved-member allowlist, and `AvailableStaticMembers.PropertyFunctionMembers` `[DynamicDependency]` keeps
