...
| [Expander.PropertyFunctionExecutor.cs](../../src/Build/Evaluation/Expander.PropertyFunctionExecutor.cs) | IL2080 | `BoundFunction.FindPublicMethodBySignature(...)` | `_receiverType.GetMethods(_bindingFlags)` | Same `_bindingFlags` no-`NonPublic` invariant; public-only bind over the bounded allowlist receiver. | Vetted |
| [Expander.PropertyFunctionExecutor.cs](../../src/Build/Evaluation/Expander.PropertyFunctionExecutor.cs) | IL3050 | `BoundFunction.LateBindExecute(...)` | `Enum.GetValues(Type)` (rooted) | `Enum.GetValues(Type)` is unreachable via property functions - no way to supply a `Type` arg (MSB4185/MSB4186) - and would fail observably if reached; proven under Native AOT by `PropertyFunctionAotTests`. | Vetted |
| [Expander.PropertyFunctionExecutor.cs](../../src/Build/Evaluation/Expander.PropertyFunctionExecutor.cs) | IL2080 | `BoundFunction.LateBindExecute(...)` | `_receiverType.GetMethods(_bindingFlags)` | Same `_bindingFlags` no-`NonPublic` invariant; public-only bind over the bounded allowlist receiver. | Vetted |
